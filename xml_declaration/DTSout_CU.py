...
class SubjectAddressDetails(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.postal_code:
            postal_code_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}PostalCode')
            postal_code_elem.text = self.postal_code
        
        if self.country_code:
            country_code_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}CountryCode')
            country_code_elem.text = self.country_code
        
        if self.country_name:
            country_name_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}CounryName')
            country_name_elem.text = self.country_name
        
        if self.region:
            region_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}Region')
            region_elem.text = self.region
        
        if self.city:
            city_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}City')
            city_elem.text = self.city
        
        if self.street_house:
            street_house_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}StreetHouse')
            street_house_elem.text = self.street_house
        
        if self.house:
            house_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}House')
...
class RFOrganizationFeatures(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.ogrn:
            ogrn_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}OGRN')
            ogrn_elem.text = self.ogrn
        
        if self.inn:
            inn_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}INN')
            inn_elem.text = self.inn
        
        if self.kpp:
            kpp_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}KPP')
...
class DTSoutSeller(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.organization_name:
            org_name_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}OrganizationName')
            org_name_elem.text = self.organization_name
        
        if self.address:
            elem.append(self.address.to_xml())
...
class DTSoutBuyer(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.rf_features:
            elem.append(self.rf_features.to_xml())
...
class DTSInvoiceDocuments(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.document_name:
            doc_name_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PrDocumentName')
            doc_name_elem.text = self.document_name
        
        if self.document_number:
            doc_number_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PrDocumentNumber')
            doc_number_elem.text = self.document_number
        
        if self.document_date:
            doc_date_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PrDocumentDate')
            doc_date_elem.text = self.document_date
        
        if self.presented_document_mode_code:
            mode_code_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}PresentedDocumentModeCode')
            mode_code_elem.text = self.presented_document_mode_code
        
        if self.position_number:
            position_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}PositionNumber')
...
class DTSBuyerSellerDependence(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.column_7a:
            column_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Column7A')
...
class DTSSellingLimitation(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.column_8a:
            column_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Column8A')
            column_elem.text = self.column_8a
        
        if self.column_8b:
            column_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Column8B')
...
class DTSAdditionalPayments(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.column_9:
            column_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Column9')
            column_elem.text = self.column_9
        
        if self.column_9a:
            column_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Column9A')
            column_elem.text = self.column_9a
        
        if self.column_9b:
            column_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Column9B')
...
class ExchangeRate(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.currency_rate:
            rate_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}CurrencyRate')
            rate_elem.text = self.currency_rate
        
        if self.currency_a3_code:
            code_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}CurrencyA3Code')
            code_elem.text = self.currency_a3_code
        
        if self.scale_number:
            scale_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}ScaleNumber')
...
class Method1CalculationBasis(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Method1CalculationBasis')
        
        if self.deal_currency_amount:
            amount_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}DealCurrencyAmount')
            amount_elem.text = self.deal_currency_amount
        
        if self.deal_currency_code:
            code_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}DealCurrencyCode')
            code_elem.text = self.deal_currency_code
        
        if self.deal_national_amount:
            amount_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}DealNationalAmount')
            amount_elem.text = self.deal_national_amount
        
        if self.deal_currency_date:
            date_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}DealCurrencyDate')
            date_elem.text = self.deal_currency_date
        
        if self.deal_currency_rate:
            rate_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}DealCurrencyRate')
            rate_elem.text = self.deal_currency_rate
        
        if self.indirect_national_payment:
            payment_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}IndirectNationalPayment')
            payment_elem.text = self.indirect_national_payment
        
        if self.basis_national_amount:
            amount_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}BasisNationalAmount')
...
class Method1AdditionalSum(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Method1AdditionalSum')
        ...
        for field_name, field_value in fields:
            if field_value is not None:
                field_elem = ET.SubElement(elem, f'{{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}}{field_name}')
...
class Method1Deduction(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}Method1Deduction')
        
        if self.building_amount:
            amount_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}BuildingAmount')
            amount_elem.text = self.building_amount
        
        if self.union_transport_charge:
            charge_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}UnionTransportCharge')
            charge_elem.text = self.union_transport_charge
        
        if self.union_tax_payment:
            payment_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}UnionTaxPayment')
            payment_elem.text = self.union_tax_payment
        
        if self.total_deduction_amount:
            amount_elem = ET.SubElement(elem, '{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}TotalDeductionAmount')
...
class DTSMethod1(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0}DTSMethod1')
        
        if self.calculation_basis:
            elem.append(self.calculation_basis.to_xml())
        
        if self.additional_sum:
            elem.append(self.additional_sum.to_xml())
        
        if self.deduction:
            elem.append(self.deduction.to_xml())
...
class DTSCUCustomsCostCalculation(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.dts_method1:
            elem.append(self.dts_method1.to_xml())
...
class DTSoutCUGoodsCustomsCost(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.exchange_rate:
            elem.append(self.exchange_rate.to_xml())
        ...
        if self.customs_cost_calculation:
            elem.append(self.customs_cost_calculation.to_xml())
...
class CommunicationDetails(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}CommunicationDetails')
        
        if self.phone:
            phone_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}Phone')
...
class SigningDetails(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:RUDeclCommonAggregateTypesCust:5.24.0}SigningDetails')
        
        if self.person_surname:
            surname_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PersonSurname')
            surname_elem.text = self.person_surname
        
        if self.person_name:
            name_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PersonName')
            name_elem.text = self.person_name
        
        if self.person_middle_name:
            middle_name_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PersonMiddleName')
            middle_name_elem.text = self.person_middle_name
        
        if self.person_post:
            post_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}PersonPost')
        ...
        if self.signing_date:
            date_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}SigningDate')
...
class SignatoryPersonIdentityDetails(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('{urn:customs.ru:RUDeclCommonAggregateTypesCust:5.24.0}SignatoryPersonIdentityDetails')
        
        if self.identity_card_code:
            code_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}IdentityCardCode')
            code_elem.text = self.identity_card_code
        
        if self.identity_card_name:
            name_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}IdentityCardName')
            name_elem.text = self.identity_card_name
        
        if self.identity_card_series:
            series_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}IdentityCardSeries')
            series_elem.text = self.identity_card_series
        
        if self.identity_card_number:
            number_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}IdentityCardNumber')
            number_elem.text = self.identity_card_number
        
        if self.identity_card_date:
            date_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}IdentityCardDate')
            date_elem.text = self.identity_card_date
        
        if self.organization_name:
            org_elem = ET.SubElement(elem, '{urn:customs.ru:RUSCommonAggregateTypes:5.24.0}OrganizationName')
...
class DTSoutCUFilledPerson(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.signatory_person_identity:
            elem.append(self.signatory_person_identity.to_xml())
...
class DeliveryTerms(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        ...
        if self.delivery_place:
            place_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}DeliveryPlace')
            place_elem.text = self.delivery_place
        
        if self.delivery_terms_string_code:
            code_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}DeliveryTermsStringCode')
...
class DTSout_CU(IXMLElement):
    ...
    def to_xml(self) -> ET.Element:
        elem = ET.Element('DTSout_CU')
        # elem.set('xmlns:RUScat_ru', 'urn:customs.ru:RUSCommonAggregateTypes:5.24.0')
        # elem.set('xmlns:cat_EDTS_cu', 'urn:customs.ru:CUESADDTSCommonAggregateTypes:5.24.0')
        # elem.set('xmlns:cat_ru', 'urn:customs.ru:CommonAggregateTypes:5.24.0')
        # elem.set('xmlns:RUDECLcat', 'urn:customs.ru:RUDeclCommonAggregateTypesCust:5.24.0')
        # elem.set('xmlns:catESAD_cu', 'urn:customs.ru:CUESADCommonAggregateTypesCust:5.24.0')
        elem.set('DocumentModeID', self.document_mode_id)
        elem.set('xmlns', 'urn:customs.ru:Information:CustomsDocuments:DTSout_CU:5.24.0')
        
        if self.document_id:
            doc_id_elem = ET.SubElement(elem, '{urn:customs.ru:CommonAggregateTypes:5.24.0}DocumentID')
        ...
        if self.seller:
            elem.append(self.seller.to_xml())
        
        if self.buyer:
            elem.append(self.buyer.to_xml())
        
        for doc in self.invoice_documents:
            elem.append(doc.to_xml())
        
        if self.buyer_seller_dependence:
            elem.append(self.buyer_seller_dependence.to_xml())
        
        if self.selling_limitation:
            elem.append(self.selling_limitation.to_xml())
        
        if self.additional_payments:
            elem.append(self.additional_payments.to_xml())
        
        if self.goods_customs_cost:
            elem.append(self.goods_customs_cost.to_xml())
        
        if self.filled_person:
            elem.append(self.filled_person.to_xml())
        
        if self.declarant:
            elem.append(self.declarant.to_xml())
        
        if self.delivery_terms:
            elem.append(self.delivery_terms.to_xml())
